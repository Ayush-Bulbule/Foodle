...
const Navbar = () => {
            ...
            if ('geolocation' in navigator) {
                navigator.geolocation.getCurrentPosition(async ({ coords }) => {
                    ...
                    try {
                        ...
                        setAddress(res.data.features[0].properties.address_line1)
    ...
    return (
        <>
            <Box position={'fixed'} zIndex={'50'} width={'100%'} flex={1} paddingX={"6"} paddingY={'4'} boxShadow={'lg'} bg={"white"} alignItems={"center"} justifyContent={"space-between"} flexDirection={"row"} borderBottom={'1.5px'} borderColor={'gray.300'}>
                <Flex width={{ base: '100%', md: '80%', xl: '80%' }} mx={'auto'} direction={"row"} justifyContent={"space-between"}>
                    <HStack verticalAlign={'center'} alignItems={"center"}>
                        ...
                        <Flex ml={'8'} alignItems={'center'} mt={1} cursor={'pointer'}>
                            ...
                            <Text size={'xs'} color={'gray.700'} fontWeight={'light'}> {address}</Text>
